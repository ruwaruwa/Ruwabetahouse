...
// const {data:client,isLoading,isError}=useQuery({
//   queryKey:['gellary'],
// queryFn: async(data)=> await Getall('gellary',data),
// onError:(er)=>{
// toast.error('sorry canot be get ',er)
// }
// })
...
//use query manages
const {data:client,isLoading,isError}=getAllquery('/gellary','gellary')
...
const {mutate,isLoading:mutateloading}=Addquery('/gellary','gellary')
...
const{mutate:updatemutate,isLoading:updateloading}=Updatequery(`/gellary/${edit}`,'gellary')
...
//   mutationFn:(id)=>Deletes(`gellary/${id}`),
//   onSuccess:()=>{
...
///dete
const {mutate:deletemutate,}=DeleteQuery(`/gellary/${DELETCLIENT}`)
//delete hoks
...
<Delete_Confirm OPEn={delethok.OPEns} TOGELER={delethok.TOggle}message={delethok.message}confirm={deleteCHeck} />
  ...
  <Typography color="text.primary">Clients</Typography>
   ...
   {/* {.....end......bredpoints} */}
    ...
    <Alert severity="info" sx={{p:2}}>our gellary  </Alert>
...
<Box>
<TextField
   label="title" variant="outlined" {...register('title',{required:true})}size="small"fullWidth
         />
...
<Typography >Loading...</Typography>
   ...
   </Box>) : <Gellarylist  clientData={client?.data} clientsUpdate={UpdateClientInfo} clientDelete={deletes} />}
  {/* //  <Box>
